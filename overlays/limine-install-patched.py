...
def get_kernel_version(kernel_path: str) -> str:
    ...
    """
    try:
        # Get the parent directory of the kernel file (the copied kernel directory)
        ...
        if version_match:
            ...
            # Check if this is a cachyos kernel by looking for "cachyos" in the name
...
def generate_config_entry(profile: str, gen: str, special: bool) -> str:
    ...
    if len(specialisation_list) > 0:
        ...
        entry += config_entry(depth, boot_spec, f'Linux {kernel_version} - Default', str(time))
    else:
        entry += config_entry(depth, boot_spec, f'Linux {kernel_version} - Generation {gen}', str(time))

    for spec, spec_boot_spec in specialisation_list:
        spec_kernel_version = get_kernel_version(spec_boot_spec.kernel)
        entry += config_entry(depth, spec_boot_spec, f'Linux {spec_kernel_version} - {spec}', str(time))
